...
2. When a test creates a program, capture the program's UUID from the POST response (`response.data.id`) and call `trackProgram(uuid)` immediately.
3. Do not write manual `afterAll` blocks for cleanup — the fixture handles teardown for every test that uses it.
4. Cleanup uses the DELETE API, not the UI: `DELETE /api/programs/<uuid>` with a Bearer token. The fixture uses `DIDAXIS_API_TOKEN` when valid; otherwise it logs in via `POST /api/auth/login` using `DIDAXIS_EMAIL` / `DIDAXIS_PASSWORD`.
5. Never hardcode the token. Never delete data the test did not create.

If auth is blocked and the agent asks for `DIDAXIS_API_TOKEN`, use Playwright MCP to capture the bearer token from a logged-in browser session, or rely on email/password login — a token in `.env` is optional.

## Reference

- **Endpoint:** `DELETE ${DIDAXIS_URL}/api/programs/<uuid>`
- **Auth:** `Authorization: Bearer ${token}` from login or optional `DIDAXIS_API_TOKEN`
