...
- **Spec:** `tests/programs.a11y.spec.ts:28` — opens New Program modal, runs axe on modal scope via `expectNoAxeViolations`
- **POM:** `pages/NewProgramModal.ts:17` — `closeButton` targets `dialog.getByRole('banner').getByRole('button')` (icon-only, no accessible name)
- **Helper:** `tests/helpers/axe-a11y.ts` — attaches violation JSON/txt on failure
...
- **Spec:** `tests/ds3-program-validation.spec.ts:239-261` — creates program with padded name `"  ${base}  "`, then if trimmed row visible asserts padded row count is 0
- **POM:** `pages/ProgramsPage.ts:70-74` — `programRow(name)` filters rows by exact text match
- **AC:** `features/DS-3.feature:28-38` TC-DS3-003 expects trimmed name in list and padded name absent — test assertion aligns with AC, but test title documents **observed** trim behavior (app may not trim)
...
Option B: document observed behavior explicitly with countProgramsNamed(padded) === 1; file separate app bug if AC trim is required.
...
- Jira filing only after human confirms app bug (jira-bug-reporter skill).
