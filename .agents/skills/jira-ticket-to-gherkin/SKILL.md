...
description: >-
  Turns a Jira ticket's acceptance criteria into structured, reviewable Gherkin
  test scenarios. Use this skill whenever the user references a Jira ticket
  (DS-1, DS-2, etc.) and asks for test cases, a test plan, scenarios, or wants
  to plan testing for a ticket — even if they don't say the word "Gherkin".
---
...
Use the Atlassian MCP server. Read tool schemas before calling (`GetMcpTools` for server `plugin-atlassian-atlassian`).

1. Resolve `cloudId` via `getAccessibleAtlassianResources`, or pass the site hostname from a Jira URL (e.g. `legionqaschool.atlassian.net`).
2. Fetch the ticket with `getJiraIssue`:
   - `issueIdOrKey`: ticket key (e.g. `DS-1`)
   - `responseContentFormat`: `markdown`
3. Parse acceptance criteria from description, custom fields, or linked Confluence pages. If ACs are embedded as Gherkin, treat them as minimum coverage — still add negative and edge scenarios.
