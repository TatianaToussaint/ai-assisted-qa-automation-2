...
|---------|-----------------|
| Delete control | Icon button `aria-label="Delete {programName}"` (trash icon) in row actions |
| Confirmation | **Native `window.confirm`** — not a Mantine modal |
| Confirm message | `Delete program "{name}"? All its semesters and courses will be removed. This cannot be undone.` |
| Accept | Browser OK → program removed from list |
| Cancel | Browser Cancel → program remains |

**Playwright:** register `page.once('dialog', …)` **before** click; do not use `Promise.all` with `waitForEvent('dialog')` (blocks on synchronous `confirm()`).
...
|----------|-----------------|
| Delete entry | Trash icon button per row |
| Confirmation UI | Native browser dialog (not in-DOM modal) |
| Confirm delete | Program row removed without page reload |
| Cancel delete | Program and description unchanged |
| Message content | Includes program name and "cannot be undone" |

**Divergence from Jira AC:** AC implies an in-app confirmation dialog; app uses `window.confirm`.
...
**Spec:** `block-05/tests/ds4-delete-program.spec.ts`  
**Last run:** 8 passed, 0 failed (Chromium, serial)

| Test Case | Automated | Notes |
|-----------|-----------|-------|
| TC-DS4-001 | Yes | Accept native confirm |
| TC-DS4-002 | Yes | Dismiss keeps program |
| TC-DS4-003 | Yes | Asserts confirm message text |
| TC-DS4-004 | Yes | Special chars in name |
| TC-DS4-005 | Yes | Delete one of multiple |
| TC-DS4-008 | Yes | Dismiss = no delete |
| TC-DS4-016 | Yes | Icon button accessible name |
| TC-DS4-018 | Yes | List updates without reload |

---

## Resolved via App Observation

1. **Delete control:** Jira says "delete icon"; app uses `Delete {programName}` action icon.
2. **Confirmation:** Native `window.confirm`, not `role="dialog"` Mantine modal.
3. **Cascade warning:** Message mentions semesters and courses removal.
